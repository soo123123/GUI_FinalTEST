...
public class SPUM_Prefabs : MonoBehaviour
{
    ...
    public AnimatorOverrideController OverrideController; // 상태별 클립 교체용

    public string UnitType;
    public List<SpumPackage> spumPackages = new List<SpumPackage>();
    public List<PreviewMatchingElement> ImageElement = new(); // 캐릭터 파츠 이미지 정보
    public List<SPUM_AnimationData> SpumAnimationData = new(); // 상태별 애니메이션 메타 데이터
    ...
    public void OverrideControllerInit() // Animator 복제 틀 만들기
    {
    ...
    public void PlayAnimation(PlayerState PlayState, int index){
        ...
        var animations =  StateAnimationPairs[PlayState.ToString()]; // 상태에 맞는 클립 리스트 찾기
        //Debug.Log(OverrideController[PlayState.ToString()].name);
        OverrideController[PlayState.ToString()] = animations[index]; // 상태 슬롯에 클립 교체
        //Debug.Log( OverrideController[PlayState.ToString()].name);
